...
public class ActionMethodsImpl implements ActionMethods {
    ...
    private final static Logger log = LogFactory.getLogger(ActionMethodsImpl.class);
     ...
     * @param interceptors Controller要执行的所有拦截器
     */
    public ActionMethodsImpl(Controller controller, List<Interceptor> interceptors) {
        ...
        for (Method m : ms) {
            if (isActionMethod(m)) {
                ClearInterceptor clearInterceptor = m.getAnnotation(ClearInterceptor.class);
                //过滤上一层的Interceptor
                List<Interceptor> list = AnnoUtils.getInterceptorsByClearInterceptor(clearInterceptor, interceptors);
                //添加方法上标注的Interceptor
